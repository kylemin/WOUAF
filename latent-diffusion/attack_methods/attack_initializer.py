...
def attack_initializer(args, is_train, device):
    ...
    aug_list = K.AugmentationSequential(
        K.CenterCrop(resolution, p = attack_prob if 'c' in args.attack else 0, keepdim=True),
    ...
    attack = transforms.Compose([
        ...
        K.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)) if 'm' in args.attack else transforms.Lambda(lambda x: x)
    ])
